...
func consume(r io.Reader,publicModel string,reasoningEnabled,chat bool,emit func([]byte)error)(Collected,error){
 ...
 acceptUsage:=func(usage *dmr.Usage)error{if usageSeen||!terminal||reasoningEnabled&&!usage.ReasoningPresent||!reasoningEnabled&&usage.ReasoningTokens!=0||usage.PromptTokens<0||usage.CompletionTokens<0||usage.ReasoningTokens<0||usage.ReasoningTokens>usage.CompletionTokens||usage.TotalTokens!=usage.PromptTokens+usage.CompletionTokens{return errors.New("invalid usage")};usageSeen=true;result.Summary.Usage=g.Usage{PromptTokens:usage.PromptTokens,CompletionTokens:usage.CompletionTokens,ReasoningTokens:usage.ReasoningTokens,TotalTokens:usage.TotalTokens};return nil}
 for scanner.Scan(){
  ...
  attachedUsage:=c.Usage!=nil&&!chat&&len(c.Choices)==1
  if c.Usage!=nil&&!attachedUsage{
   if len(c.Choices)!=0{return result,errors.New("invalid usage chunk")};if e:=acceptUsage(c.Usage);e!=nil{return result,e};usage,_:=json.Marshal(result.Summary.Usage);raw["usage"]=usage
  }else{
   ...
   if attachedUsage{if e:=acceptUsage(c.Usage);e!=nil{return result,e};raw["usage"]=json.RawMessage("null")}
  }
  if emit!=nil{model,_:=json.Marshal(publicModel);raw["model"]=model;if c.Usage==nil{raw["usage"]=json.RawMessage("null")};out,e:=json.Marshal(raw);if e!=nil{return result,e};if e=emit(out);e!=nil{return result,e};if attachedUsage{raw["choices"]=json.RawMessage("[]");usage,_:=json.Marshal(result.Summary.Usage);raw["usage"]=usage;out,e=json.Marshal(raw);if e!=nil{return result,e};if e=emit(out);e!=nil{return result,e}}}
 }
