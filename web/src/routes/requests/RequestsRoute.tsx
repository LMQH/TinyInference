...
export default function RequestsRoute() {
  ...
  return (
    <section className="page">
      <header className="page-header"><h1>请求与队列</h1><p>观察唯一活跃请求、后端权威 FIFO 等待顺位和不含正文的最近请求元数据。</p></header>

      <section className="section" aria-labelledby="active-heading">
        <div className="section-heading"><div><h2 id="active-heading">活跃请求</h2><p>最多 1 个；没有活跃项不代表模型已就绪。</p></div></div>
        ...
        ) : snapshot.data ? <p className="empty-state">当前没有活跃请求。</p> : null}
      </section>

      <section className="section" aria-labelledby="waiting-queue-heading">
        <div className="section-heading"><div><h2 id="waiting-queue-heading" tabIndex={-1}>完整等待队列</h2><p>当前等待深度 {snapshot.data?.queue.depth ?? '无法获取'} / {snapshot.data?.queue.capacity ?? 20}。</p></div></div>
      ...
      <section className="section" aria-labelledby="history-heading">
        <div className="section-heading"><div><h2 id="history-heading">最近请求元数据</h2><p>固定按创建时间与请求标识倒序，不提供客户端排序或正文搜索。</p></div></div>
        ...
        {requests.data?.items.length === 0 && <p className="empty-state">当前没有请求记录。</p>}
        {requests.data && requests.data.items.length > 0 && (
          <div className="table-scroll" role="region" aria-labelledby="history-heading" tabIndex={0}>
            <p className="table-description">表格可横向滚动。顺序由后端固定，不可排序。</p>
