...
if len(sys.argv)!=2 or not re.fullmatch(r'mini-inference-\d{8}T\d{6}Z-6\.dump',sys.argv[1]):
    raise SystemExit('an exact schema-6 backup basename is required')
root=pathlib.Path(__file__).resolve().parents[2]
out=root/'var'/'artifacts'/'restore-selection'
